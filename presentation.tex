		...
		\begin{column}{0.45\textwidth} % Slightly increased width
			\begin{block}{Key Findings}
				...
				\begin{itemize}\setlength\itemsep{3pt}
					...
					\item \texttt{IncomeInvestment}: \textbf{moderate imbalance} ($38.4\%$, $\approx 2.3{:}1$)
					\item $\chi^2$ test: $p = 0.44 \gg 0.05$ $\Rightarrow$ targets are independent.
				\end{itemize}
			\end{block}
			\begin{alertblock}{Modeling Implication}
				\footnotesize Naive all-zero baseline gives $\sim 70\%$ accuracy, so accuracy alone is misleading. We optimize \textbf{F$_{0.5}$} and model the two targets as outputs.
			\end{alertblock}
		\end{column}
...
\begin{frame}
	\frametitle{LightGBM Model Insights: Income vs Accumulation}
	\begin{columns}[T]

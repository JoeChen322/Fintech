...
\begin{frame}[t]
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{column}[t]{0.58\textwidth}
			\raggedright\textbf{Project Flow}\par
			\vspace{0.05cm}
			\begin{center}
				\begin{tikzpicture}[
						node distance=1.05cm,
						box/.style={draw=darkblue, thick, fill=lightblue!20, text width=6.2cm, minimum height=1.0cm, align=center, rounded corners=2mm, font=\small},
						arrow/.style={-latex, thick, darkblue, line width=1.1pt}
					]
					\node[box] (eda) {\textbf{1. Exploratory Data Analysis}\\ \scriptsize Data quality, distributions, outliers};
					\node[box, below=of eda] (ml) {\textbf{2. Classification \& Model Selection}\\ \scriptsize Baselines, tuning, explainability, DCN comparison};
					\node[box, below=of ml] (rec) {\textbf{3. Recommendation Engine}\\ \scriptsize SVD/Autoencoder ranking and product matching};
...
\begin{frame}
	...
	\begin{columns}[c] % Changed [T] to [c] for vertical centering
		\begin{column}{0.45\textwidth} % Slightly increased width
			\begin{block}{Key Findings}
				\small
				\begin{itemize}\setlength\itemsep{3pt}
					\item \texttt{AccumulationInvestment}: \textbf{balanced} ($51.3\%$, $\approx 1{:}1$)
					\item \texttt{IncomeInvestment}: \textbf{moderate imbalance} ($38.4\%$, $\approx 1.6{:}1$)
					\item $\chi^2$ test: $p = 0.44 \gg 0.05$ $\Rightarrow$ targets are independent.
				\end{itemize}
			\end{block}
			\begin{alertblock}{Modeling Implication}
				\footnotesize Naive all-zero baseline gives $\sim 62\%$ accuracy, so accuracy alone is misleading. We optimize \textbf{F$_{0.5}$} and model the two targets as outputs.
			\end{alertblock}
		\end{column}

		\begin{column}{0.52\textwidth} % Adjusted width
			\centering
			\includegraphics[width=\linewidth]{figures/target_distribution_crosstab.png}\\
			\vspace{0.2cm} % Added a little spacing between image and caption
			\footnotesize Target prevalence and cross-tab evidence
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.64\textwidth}
			\centering
			\includegraphics[width=0.75\linewidth]{figures/violinplots_by_target.png}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{block}{Final Selection}
		For the final recommendation pipeline, we use \textbf{Tuned RF} for \textbf{Income Investment} and \textbf{Tuned LightGBM} for \textbf{Accumulation Investment}, balancing precision and task-specific predictive strength.
	\end{block}
\end{frame}
...
\begin{frame}
	\frametitle{Model Insights: Income (RF) vs Accumulation (LGBM)}
	\begin{columns}[T]
		\begin{column}{0.4\textwidth}
			\centering
			\scriptsize Income Investment\\[0.05cm]
			\includegraphics[width=0.95\linewidth]{figures/extracted_img_4.png}\\
			\scriptsize SHAP Feature Importance\\[0.05cm]
			\includegraphics[width=0.7\linewidth]{figures/extracted_img_5.png}\\
			\scriptsize SHAP Beeswarm Plot
		\end{column}
		\begin{column}{0.4\textwidth}
			\centering
			\scriptsize Accumulation Investment\\[0.05cm]
			\includegraphics[width=0.95\linewidth]{figures/extracted_img_6.png}\\
			\scriptsize SHAP Feature Importance\\[0.05cm]
			\includegraphics[width=0.7\linewidth]{figures/extracted_img_7.png}\\
			\scriptsize SHAP Beeswarm Plot
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	\frametitle{LIME, PDP and ICE for RF income investment}
	\begin{columns}[T,onlytextwidth]
		\begin{column}{0.49\textwidth}
			\centering
			\includegraphics[width=\linewidth,height=0.68\textheight,keepaspectratio]{figures/extracted_img_15.png}\\
		\end{column}
		\begin{column}{0.49\textwidth}
			\centering
			\scriptsize \textbf{LIME Explanation (RF): Income Investment}\\[0.05cm]
			\includegraphics[width=\linewidth,height=0.64\textheight,keepaspectratio]{figures/extracted_img_16.jpg}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	\frametitle{DCN Overview and Selection Decision \cite{guo_explainable_2023}}
	\begin{columns}[T]
		\begin{column}{0.48\textwidth}
			\centering
			\resizebox{\linewidth}{!}{
		...
		\begin{column}{0.52\textwidth}
			...
			\resizebox{\linewidth}{!}{%
				\begin{tabular}{lccccc}
					...
					\midrule
					Income        & 0.776        & 0.827         & 0.547        & 0.659       & 0.765        \\
					Accum.        & 0.795        & 0.851         & 0.702        & 0.769       & 0.848        \\
					\bottomrule
				\end{tabular}%
			}

			\vspace{0.25cm}
			\begin{block}{Why Not Selected}
				\begin{itemize}
					\item Slight gain for \textbf{Income} vs tuned RF for F1, but not for precision.
					\item Lower performance for \textbf{Accumulation} vs tuned LightGBM.
					\item Lower interpretability than tree-based models with SHAP/LIME.
				\end{itemize} 
			\end{block}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	\begin{table}
		\centering
		\scriptsize
		\caption*{\footnotesize Holdout validation results (production models: SVD $n{=}6$, AE $\text{dim}{=}8$)}
		\resizebox{0.55\textwidth}{!}{%
			\begin{tabular}{lcccc}
				\toprule
				\textbf{Model} & \textbf{RMSE}  & \textbf{Precision@3} & \textbf{Recall@3} & \textbf{NDCG@3} \\
				\midrule
				Truncated SVD  & 0.664          & 0.930                & 0.930             & 0.977           \\
				Autoencoder    & \textbf{0.016} & \textbf{0.994}       & \textbf{0.994}    & \textbf{0.999}  \\
				\bottomrule
			\end{tabular}%
		}
	\end{table}
	...
	\begin{columns}
		\begin{column}{0.45\textwidth}
			\centering
			\includegraphics[width=0.68\linewidth]{figures/svd_tuning.png}\\
			\small SVD Tuning
		\end{column}
		\begin{column}{0.5\textwidth}
			\centering
			\includegraphics[width=0.68\linewidth]{figures/autoencoder_tuning.png}\\
			\small Autoencoder Tuning
		\end{column}
	\end{columns}

	\vspace{0.2cm}
	\begin{block}{Financial Perspective}
		\footnotesize The Autoencoder's superior ranking quality (higher Precision@3/Recall@3/NDCG@3) supports better product-client matching, which can improve conversion and fee revenue while reducing suitability risk and long-term churn from mis-sold recommendations.
	\end{block}
\end{frame}

...
\begin{frame}{Part 1 $\cdot$ Isolation Forest: the warning signal in action}
	\centering
	\includegraphics[width=\textwidth,height=0.64\textheight,keepaspectratio]{figures/if_01_temporal_anomaly_score.pdf}

	\vfill
	\footnotesize
	Score is \emph{inverted} (lower $=$ more abnormal); it dips below the threshold at every
	major stress -- 2008, 2011, 2020. \textbf{Top-$K$ feature selection lifts test recall
		$48\%\to83\%$} ($F_2\;0.50\to0.76$) -- the strongest recall of any detector.
\end{frame}

% =========================================================================
\begin{frame}{Part 1 $\cdot$ What drives the signal}
	\begin{columns}[c,onlytextwidth]
		\column{0.54\textwidth}
		\centering
		\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{figures/if_02_feature_importance.pdf}
		\column{0.42\textwidth}
		\small Permutation importance (test-set ROC-AUC drop):
		\begin{itemize}\footnotesize
			\item Top signals are \textbf{52-week drawdowns}, \textbf{VIX momentum}, and
			      \textbf{cross-asset correlations} with US equity.
			\item The forest keys on \emph{classic crisis tells}, not artefacts.
			\item Risk-managed, \textbf{Top-$K$ IF cuts out-of-sample drawdown to
				      $-13.0\%$} (vs.\ $-27.7\%$ buy \& hold) -- the deepest cut of all five.
		\end{itemize}
	\end{columns}
\end{frame}
